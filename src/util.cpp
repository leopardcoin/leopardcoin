...
const char * const BITCOIN_CONF_FILENAME = "leopardcoin.conf";
const char * const BITCOIN_PID_FILENAME = "leopardcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "leopardcoin" is a composite category enabling all LeopardCoin-related debug output
                if(ptrCategory->count(std::string("leopardcoin"))) {
...
#else
    const char* pszModule = "leopardcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LeopardCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LeopardCoin
    // Mac: ~/Library/Application Support/LeopardCoin
    // Unix: ~/.leopardcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LeopardCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LeopardCoin";
#else
    // Unix
    return pathRet / ".leopardcoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty leopardcoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override leopardcoin.conf
