...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete LeopardCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no LeopardCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old LeopardCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected LeopardCoin 0.7 datadir."
    ;;
  3)
    echo "Detected LeopardCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected LeopardCoin 0.8 datadir."
    ;;
esac
