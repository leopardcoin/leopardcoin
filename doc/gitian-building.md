...
*Setup instructions for a Gitian build of LeopardCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LeopardCoin
...
- [Building LeopardCoin Core](#building-leopardcoin-core)
...
Clone the git repositories for LeopardCoin Core and Gitian.
...
in the LeopardCoin Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building LeopardCoin Core
----------------

To build LeopardCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the LeopardCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/leopardcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/leopardcoinpay/leopardcoin
...
For example:
```bash
URL=https://github.com/crowning-/leopardcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit leopardcoin=${COMMIT} --url leopardcoin=${URL} ../leopardcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit leopardcoin=${COMMIT} --url leopardcoin=${URL} ../leopardcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit leopardcoin=${COMMIT} --url leopardcoin=${URL} ../leopardcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the leopardcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../leopardcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/leopardcoin
SIGPATH=/some/root/path/leopardcoin-detached-sigs

./bin/gbuild --url leopardcoin=${BTCPATH},signature=${SIGPATH} ../leopardcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/leopardcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/leopardcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/leopardcoin-osx-build.assert
```
...
[leopardcoin/gitian.sigs](https://github.com/leopardcoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
