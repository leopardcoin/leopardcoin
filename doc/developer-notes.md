...
LeopardCoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 6211 for connections and services them.
...
However, in open source software such as LeopardCoin Core, where everyone uses
...
pay attention to for reviewers of LeopardCoin Core code.

General LeopardCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: LeopardCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against LeopardCoin Core so that they can be integrated
